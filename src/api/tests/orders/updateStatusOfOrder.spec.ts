...
orderDraftWithDeliveryStatus.describe(
  '[API][Orders] Draft with delivery - In Process Orders/Canceled',
  () => {
    let token = '';
...
orderCanceledStatus.describe(
  '[API][Orders] Canceled - Draft/In Process',
  () => {
    let token = '';
...
orderInProcessStatus.describe(
  '[API][Orders] In Process - Draft/Canceled/In Process/Partially Received/Received',
  () => {
    let token = '';
      ...
      orderInProcessStatus(
        ...
        async ({ orderData, ordersController }) => {
          ...
          validateResponse(
            ...
            STATUS_CODES.BAD_REQUEST,
            false,
            ERROR_MESSAGES.CANT_REOPEN_ORDER,
          );
        },
      );
      ...
      orderInProcessStatus(
        ...
        async ({ orderData, ordersController, ordersApiService }) => {
          ...
          validateResponse(
            ...
            false,
            ERROR_MESSAGES.ORDER_NOT_FOUND_WITH_ID(nonExistentOrder),
          );
        },
      );
    ...
    orderPartiallyReceivedStatus.describe(
      '[API][Orders] Partially Received - Received',
      () => {
        let token = '';
