...
test.describe('[UI] [Sales Portal]', () => {
  ...
  test.describe('Orders list', () => {
    test(
      'Should display Orders List title',
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
        await ordersPage.waitForOpened();
    ...
    test(
      'Should navigate to Create Order page after clicking button',
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
    ...
    test(
      'Should filter orders by search input',
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
    ...
    test(
      ...
      async ({ homeUIService, ordersPage }) => {
        ...
        await homeUIService.openModule('Orders');
        await ordersPage.waitForOpened();
        const initialDirection = await ordersPage.getCurrentSortDirection(
          OrdersListColumnForSorting.OrderNumber,
        );
        expect(initialDirection).toBe('none');
        await ordersPage.sortColumnBy(
          OrdersListColumnForSorting.OrderNumber,
          'asc',
        );
        const finalDirection = await ordersPage.getCurrentSortDirection(
          OrdersListColumnForSorting.OrderNumber,
        );
    ...
    test(
      ...
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage, confirmationModal }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
        await ordersPage.waitForOpened();
    ...
    test(
      'Should change items per page to 25',
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
    ...
    test(
      'Should navigate to a specific page number',
      { tag: [TAGS.SMOKE] },
      async ({ homeUIService, ordersPage }) => {
        await homeUIService.openAsLoggedInUser();
        await homeUIService.openModule('Orders');
        ...
        await expect(
          ordersPage.getPageByNumber(targetPageNumber),
    ...
    test(
      ...
      async ({ homeUIService, orderDetailsPage, ordersPage }) => {
        await test.step('Navigate to an Order Details Page', async () => {
          ...
          const orderDetails =
            await orderDetailsPage.topPanel.getOrderDetails();
